            ...
            <div className={styles.blooksHolder}>
                {packs.filter((pack: Pack) => !pack.hidden).sort((a: Pack, b: Pack) => a.name.localeCompare(b.name)).map((pack: Pack, i: number) => (
                    <div className={styles.setHolder} key={i}>
                        ...
                        <div className={styles.setBlooks}>
                            {pack.blooks.map((blook: any, i: number) => (
                                <div className={styles.blookContainerHolder} key={i} role="button" tabIndex={0}>
                                    {user.blooks[blook.id] > 0 ? (
                                        <>
                                            <div className={`${styles.blookContainerInside} ${styles.blookInside}`} onClick={() => { setRightBlook(allBlooks.find((blook: any) => blook === blook)); setMLeftShowing(false); }} key={i}>
                                        ...
                                        <>
                                            <div className={`${styles.blookContainerInside} ${styles.blookInside} ${styles.lockedBlook}`} key={i}>
                                                <img className={styles.blook} src={constructBlook(blook.id)} draggable="false" />
                                            </div>
